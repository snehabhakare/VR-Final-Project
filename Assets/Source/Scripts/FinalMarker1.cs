...
public class FinalMarker1 : MonoBehaviour
{
    ...
    public void Write()
    {
        script.writer3.WriteLine("Table1 Marker," + (script.TimeElapsed - start).ToString() + "," + (transform.localPosition.x).ToString() + "," + (transform.localPosition.z).ToString());
    }
    ...
    void InteractableObjectGrabbed(object sender, InteractableObjectEventArgs e)
    {
        if (first) {
            ...
            script.writer1.WriteLine(script.TimeElapsed.ToString() + " : Grabbed Table1 marker");
            first = false;
        }
    }
